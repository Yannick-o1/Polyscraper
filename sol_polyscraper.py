...
def get_binance_data_and_ofi():
    ...
    try:
        ...
        resp = requests.get("https://api.binance.com/api/v3/trades", 
                           params={"symbol": "SOLUSDT", "limit": 1000}, timeout=5)
        resp.raise_for_status()
        trades = resp.json()
        
        if not trades:
            # Fallback to ticker price if no trades are returned
            price_resp = requests.get("https://api.binance.com/api/v3/ticker/price", params={"symbol": "SOLUSDT"})
...
def init_database():
    ...
    # Check if the sol_usdt_spot column exists and add it if it doesn't
...
def get_current_market_token_id():
    """Find the token ID for the Solana market that corresponds to the current date/time"""
...
def collect_data_once():
    ...
    try:
        ...
        if best_bid and best_ask:
            ...
            if sol_price is not None and p_start is not None:
                delta = sol_price - p_start
                abs_delta = abs(delta)
                
                print(f"Delta calculation: current_price={sol_price:.2f}, p_start={p_start:.2f}, delta={delta:.2f}")
                ...
                if abs_delta > DELTA_THRESHOLD:
                    ...
                    if success:
                        print(f"✅ Order placement successful for SOL")
                    else:
                        print(f"❌ Order placement failed for SOL")
                else:
