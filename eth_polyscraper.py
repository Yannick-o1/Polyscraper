...
BANKROLL_USAGE_FRACTION = 0.95 # Fraction of total bankroll to use for sizing
...
if POLYMARKET_AVAILABLE and TRADING_ENABLED and POLYMARKET_PRIVATE_KEY and POLYMARKET_PROXY_ADDRESS:
    try:
        ...
        print("Polymarket client initialized successfully for ETH")
    except Exception as e:
        print(f"Failed to initialize Polymarket client for ETH: {e}")
        polymarket_client = None
elif POLYMARKET_AVAILABLE:
    if TRADING_ENABLED:
        print("Polymarket credentials not provided for ETH. Set POLYMARKET_PRIVATE_KEY and POLYMARKET_PROXY_ADDRESS environment variables to enable order placement.")
else:
    print("Polymarket functionality disabled for ETH.")
...
def get_binance_data_and_ofi():
    """Fetch live ETH/USDT price and calculate order flow imbalance from recent trades."""
    try:
        ...
        print(f"\nError getting live Ethereum data: {e}")
        return None, None
...
def parse_market_datetime(market_slug, market_question):
    ...
    """
    # Pattern 1: ethereum-up-or-down-{month}-{day}-{hour}{am|pm}-et
    match = re.match(r'ethereum-up-or-down-(\w+)-(\d+)-(\d+)(am|pm)-et', market_slug)
    # Pattern 2: ethereum-up-or-down-{month}-{day}-{hour}-{am|pm}-et
    if not match:
        match = re.match(r'ethereum-up-or-down-(\w+)-(\d+)-(\d+)-(am|pm)-et', market_slug)
...
def is_ethereum_hourly_market(market):
    """Checks if a market is a Ethereum hourly market based on its question."""
    q = market["question"].lower()
    return "ethereum up or down" in q and ("pm et" in q or "am et" in q)

def update_markets_csv():
    """
    Fetches all Ethereum hourly markets and saves them to a CSV file.
    """
    market_data = []
    print("--- Updating market list ---")
    print("Searching for Ethereum hourly markets...")
    ethereum_market_count = 0

    for m in get_all_markets():
        if not is_ethereum_hourly_market(m):
            continue
        
        ethereum_market_count += 1
        ...
        print(f"Found market {ethereum_market_count}: {slug}")
    ...
    if not market_data:
        print("No Ethereum hourly markets found.")
        return
    ...
    try:
        ...
        print(f"Found {ethereum_market_count} Ethereum hourly markets")
...
def get_current_market_token_ids():
    """Find the token IDs for the Ethereum market that corresponds to the current date/time"""
...
def collect_data_once():
    ...
    try:
        ...
        if model is not None and eth_price is not None:
        ...
        if best_bid and best_ask:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO polydata (timestamp, market_name, token_id, best_bid, best_ask, eth_usdt_spot, ofi, p_up_prediction)
                VALUES (:timestamp, :market_name, :token_id, :best_bid, :best_ask, :eth_usdt_spot, :ofi, :p_up_prediction)
            ''', data_row)
            conn.commit()
            conn.close()
...
def get_positions_from_data_api():
    ...
    if not POLYMARKET_PROXY_ADDRESS:
        ...
        return None
    try:
        # Correct endpoint path without version prefix
...
def manage_positions(delta, token_id_yes, token_id_no, price_yes, price_no, best_bid_yes, best_ask_yes):
    ...
    print("--- End Position Management (ETH) ---")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(
        description="Polymarket scraper for Ethereum hourly markets.",
