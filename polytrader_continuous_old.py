...
def get_or_set_hour_start_price(currency, current_price):
    """Get the exact hour start price using historical data for precision."""
    try:
        ...
        # No cache - need to find the true hour start price
        # Look for the first data point at or after the hour boundary
        hour_start_str = current_hour_utc.strftime('%Y-%m-%d %H:%M:%S')
        
        # Try to get the earliest price from this hour from the database
        cursor.execute(f"""
        ...
        if db_result and db_result[0] is not None:
            # Found historical data for this hour - use it
            p_start = db_result[0]
        else:
            # No historical data yet for this hour
            # Get the last known price from the previous hour as best estimate
            cursor.execute(f"""
                SELECT {config['db_column']} FROM polydata 
                WHERE timestamp < ? AND {config['db_column']} IS NOT NULL
            ...
            p_start = prev_result[0] if prev_result and prev_result[0] is not None else current_price
        ...
        # Fallback to current price if anything fails
...
def manage_positions_dynamic(currency, delta, token_id_yes, token_id_no, price_yes, price_no, best_bid_yes, best_ask_yes):
    ...
    try:
        ...
        # 4. Check position alignment and trade feasibility
        min_trade_cost = MINIMUM_ORDER_SIZE_SHARES * max(price_yes, 1-price_yes)
        
        # Check if positions are truly aligned (within small tolerance)
        if abs(position_adjustment) < 0.001:  # Truly aligned
            return {
                "executed": False, 
        ...
        # Check if adjustment needed is smaller than minimum trade size
        if abs(position_adjustment) < MINIMUM_ORDER_SIZE_SHARES:
            return {
                "executed": False,
                "reason": "adjustment_too_small_for_minimum",
                "current_pos": f"{current_net:+.2f}",
                "target_pos": f"{target_net:+.2f}",
                "adjustment": f"{position_adjustment:+.2f}",
                "min_shares": f"{MINIMUM_ORDER_SIZE_SHARES}"
            }
...
def trade_currency_once(currency):
    ...
    try:
        ...
        if trade_result:
            ...
            if trade_result["executed"]:
            ...
            elif trade_result.get("reason") == "adjustment_too_small_for_minimum":
                min_shares = trade_result.get('min_shares', '5')
                print(f"    🔹 TOO SMALL: Need Δ{adjustment} shares, minimum {min_shares}")
                print(f"    📊 Position: {current_pos} → {target_pos} (can't adjust)")
                ...
                print(f"    💸 TOO EXPENSIVE: Need {min_cost} for 5 shares, have {balance}")
